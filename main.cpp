...
void main(){
  ...
  printf("\n 8. Modificar Registro Estatal."); //No funciona, elimina el registro estatal.
  printf("\n 9. Modificar Centro Electoral. "); //Funciona
  printf("\n 10. Modificar Persona."); //Funciona
  printf("\n 11. Reporte General."); //funciona
  printf("\n 12. Reporte por Registro Estatal."); //funciona
  printf("\n 13. Reporte por Centro Electoral."); //funciona
  printf("\n 14. Buscar persona por cedula. "); //funciona
  printf("\n 15. Sinonimos."); // funciona
  printf("\n 16. Cambiar de Centro Electoral a una persona."); // funciona
  printf("\n 17. Buscar por rango de edad.");
  printf(" 0. Salir y guardar. \n\n");
  printf("Indique opcion (1-17): ");
  op = 0;
  scanf("%i", &op);
  system("cls");
  printf("\n\n");
  ...
  case 6:
    ...
    scanf("%s", code);
    if(centerExists(c, code)){
      printf(" Codigo Centro electoral destino de los electores");
      char codeto[9];
      scanf("%s", codeto);
      if(centerExists(c, codeto))
        DeleteCenter( r, code, codeto );
      else
        printf("   El centro electoral destino no existe!\n");
    }
    else
      printf("   El centro electoral a eliminar no existe\n");
    //flag = false;
    //seachDeleteCenter(&r, code, flag);
    break;

  case 7:
    printf(" Codigo Region Estatal para eliminar \n (Guarda votantes en otra Region Estatal.):");
    scanf("%s", codeR);
    if (registerExists(r,codeR))
      DeleteRegister(c, &r, codeR);
    else
      printf("El codigo de registro estatal no existe!\n");
    //flag = false;
  ...
  case 13:
    printf(" Reporte por Centro Electoral. \n");
    printf(" Codigo Centro Electoral para reporte: \n");
    scanf(" %s", codeR);
    seachReCe(r, codeR);
    break;
  ...
  case 17:
    int max = 0, min = 0;
    printf("\n%-10s - %-60s", "", "Introduce el valor minimo para el rango\n");
    scanf("%i", &min);
    printf("\n%-10s - %-60s", "", "Introduce el valor maximo para el rango\n");
    scanf("%i", &max);
    (max >= min) ? peopleRangAge( p, min, max ) : printf("\n\nintroduce valores correctos\n\n");
    break;
    }
    if (op)
    {
