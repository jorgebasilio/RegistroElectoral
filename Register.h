...
Register *KeyToRegister( key *data ){
  ...
  int cont = 0;
  while(cont < 4 && (data != NULL))
  {
    if (strcmp(data->clave,"nombre") == 0)
    {
      strcpy(registr->name, data->valor);
      cont++;
    }
    if (strcmp(data->clave,"direccion") == 0)
    {
      strcpy(registr->address, data->valor);
      cont++;
    }
    if (strcmp(data->clave,"codigo") == 0)
    {
      strcpy(registr->code, data->valor);
      cont++;
    }
    if (strcmp(data->clave,"telefono") == 0)
    {
      strcpy(registr->number, data->valor);
      cont++;
    }

    data = data->next;
  }
  if (cont == 4) return(registr);
  else return(NULL);
}


index *IndexRegister( key **data, Register *center ){
  if(*data != NULL){
    while(*data){
...
bool registerExists( Register *data, char code[]){
  while(data != NULL){
